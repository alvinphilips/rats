pub use ratser::{Triangle, TriangleList, Vertex, VertexComponent};
...
impl Serialize for Vertex {
    fn get_from_str(input: &str) -> Self {
        let components: Vec<VertexComponent> = input
            .split(',')
            .filter_map(|val| val.parse::<VertexComponent>().ok())
            .collect();
