        ...
        for y in 0..self.height {
            for x in 0..self.width {
                let point = Vertex::new(
                    self.bounds.min.x + x as VertexComponent,
                    self.bounds.min.y + y as VertexComponent,
                );
    ...
    pub fn render(&self) {
        for line in &self.texture {
            for pixel in line {
                print!(
                    "{}",
                    self.depth_symbols.get(*pixel as usize).unwrap_or(&'?')
                );
            }
            println!();
        }
...
impl Default for Renderer {
    fn default() -> Self {
        ...
        Self {
            ...
            depth_test_mode: Default::default(),
        }
    }
}
